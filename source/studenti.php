...
<div class="topnav">
  ...
  <a class="active" href="studenti.php">Studenti</a>
...
<tr>
<th onclick="sortTable(0)">CodSt</th>
...
<button class="btn btn-success" id= "btnXls" onclick="exportTableToExcel('myTable', 'studenti')" >Xls file</button>
...
<input type="submit" name="Load" class="btn btn-primary" value="Load">
</form>
...
$sql    = "SELECT CodSt,Nume,Prenume,Cetatenie,DataN from student";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while ($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["CodSt"] . "</td><td>" . $row["Nume"] . "</td><td>" . $row["Prenume"] . "</td><td>" . $row["Cetatenie"] . "</td><td>" . $row["DataN"] . "</td></tr>";
    }
    echo "</table>";
} 
$conn->close();
...
if($_POST["filter"]=="roman"){
$conn = mysqli_connect("localhost", "", "", "");
// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}
$sql    = "SELECT CodSt,Nume,Prenume,Cetatenie,DataN from student where cetatenie='roman'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while ($row = $result->fetch_assoc()) {
...
if($_POST["filter"]=="strain"){
$conn = mysqli_connect("localhost", "", "", "");
// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}
$sql    = "SELECT CodSt,Nume,Prenume,Cetatenie,DataN from student where  not cetatenie='roman' ";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while ($row = $result->fetch_assoc()) {
        ...
        function Export() {
            html2canvas(document.getElementById('myTable'), {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("Studenti.pdf");
                }
            });
        }
    </script>
